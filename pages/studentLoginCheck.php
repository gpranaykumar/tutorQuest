...
if (isset($_POST['uname']) && isset($_POST['password'])) {
	...
	if (empty($uname)) {
		...
		$sql = "SELECT * FROM student_data WHERE uname='$uname' AND password='$pass'";
		...
		if (mysqli_num_rows($result) === 1) {
            ...
            if ($row['uname'] === $uname && $row['password'] === $pass) {
				...
				if($row['status'] === '0'){
					header("Location: studentLogin.php?error=Account Not Activated - Under Verification");
		        	exit();
				}
				if($row['status'] === '3'){
					header("Location: studentLogin.php?error=Account Blocked By Admin");
		        	exit();
				}
